...
class CometBlitzAction extends PlayerAction with MovementHelper {
  ...
  class TailCreator(ownerId: Long, angle: Double) extends GameStateUpdater {
    ...
    def update(context: GameStateUpdateContext) = {
      ...
      for (i <- 0 to 50) {
        ...
        tail.movement = getLineMovement(angle, spawnPosition, 67 + i / 5)
  ...
  def doAction(owner: GameObject, target: MapPosition, context: GameStateUpdateContext) {
    ...
    val movement = getLineMovement(angle, orginalPosition, 75)
    owner.movement = movement
    context.addUpdater(new TailCreator(owner.id, angle))
  }
